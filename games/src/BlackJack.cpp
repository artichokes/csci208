...
BlackJack::BlackJack() : deck(Deck()), house_hand(0), player_hand(0) {
  this->deck.shuffle();
...
void BlackJack::initial_deal() {
  Card house1 = this->deck.draw(), house2 = this->deck.draw();   // House cards
  this->house_hand += cardToPoints(house1);
  this->house_hand += cardToPoints(house2);
  Card player1 = this->deck.draw(), player2 = this->deck.draw(); // Player cards
